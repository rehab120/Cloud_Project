...
namespace Cloud_Project.Application.Command.LoginMerchant
{
    ...
    public class LoginHandler : IRequestHandler<LoginCommand, LoginResult>
    {
        ...
        public async Task<LoginResult> Handle(LoginCommand command, CancellationToken cancellationToken)
        {
            var (success, token, errors) = await identityService.LoginAsync(command.UserName, command.Password);
            return new LoginResult(success, token, errors);
        }
    }
}
